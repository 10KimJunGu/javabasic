...
//		대입 연산자(=)를 이용해서 변수가 복사되는 과정을 이해해보자 
//		'변수의 복사' 라는 말의 의미는 무엇일까.. 바로 변수에 포함된 메모리값이다.
//		그런데 기본 자료형과 참조 자료형이 저장되는 메모리 값의 의미가 다르다.
//		따라서 둘 사이에 차이가 발생된다.
...
//		이제 참조 자료형을 살펴보자. 참조 자료형은 실제 데이터 값이 아닌 저장된 위치값을 가지고 있다.
...
//		하나의 참조 변수를 이용해서 데이터를 수정하면 다른 참조변수가 가리키는 데이터도 변하게 된다.
		...
		// 배열의 길이
//		.length    '배열 참조 변수가 가리키는 곳에 배열 객체가 있고 그 배열의 갯수를 가져와라'라는 뜻임
		
//		int[] a = new int[] {3,4,5,6,7};
//		System.out.println(a.length);  //배열의 갯수 뽑기
//		
...
//					배열의 선언을 보면 차원이 1개식 늘어날대마다 대괄호가 1개씩 늘어난다.
//					배열의 위치 표현은 세로방향으로 숫자가 늘어나는 행 row번호
//					가로방향으로 숫자가 늘어나는 열column번호로 구성된다.
