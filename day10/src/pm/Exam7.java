...
/*		
		...
		최초 try 구문이 실행되어 만약 예외가 발생하지 않으면 catch는 실행되지 않는다. 
		그리고 finally 블록 구문을 실행한다.
		그런데 try 구문이 실행되어 만약 예외가 발생하면 자바가 먼저 인지하여 예외 타입의 객체를 생성하여 
		catch의 매개 변수로 전달한다.
		이것은 마치 자바가 catch()라는 이름의 매서드를 호출하는 것과 비슷하다. 만약 자바가 생성해서 넘겨준
		객체 타입을 catch블록이 처리할 수 없을때 즉, 해당 객체를 받을 catch() 블록 존재하지 않으면
		예외처리가 되지 않는다.
...
//		이것은 또 마치 if-else if- else 구문에서 조건식을 위에서부터 검사하는 것과 같다.
		...
		//1개의 catch()블록으로 2개의 예외를 동시에 처리하도록 통합해보자. 둘 중 어떤 예외가 발생해도
		//모두 하나의 예외 처리로 해결할 수 있다.
		
//		try {
//			System.out.println(3/1);
//			int num = Integer.parseInt("10A");
//		}catch(ArithmeticException | NumberFormatExceptione e) {
