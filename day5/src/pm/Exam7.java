...
/*		클래스와 객체
		...
		자바는 객체지향언어로 클래스, 추상클래스, 인터페이스와 같은 객체 지향적 문법요소를 사용하여 프로그램을 구성하게 된다.
		자바를 제대로 사용하려면 이러한 문법 요소를 먼저 이해하는것이 아주 중요하다
		
		1. 클래스의 개념
		객체는 object 사용할 수 있는 실체를 의미하고 클래스는 그 객체를 만드는 설계도라고 볼 수 있다.
		따라서 한 개의 클래스로 여러개의 객체를 만들 수 있다.
		...
		배열 : 변수들이 많아지면 관리가 힘들다. 배열을 사용하면 같은 자료형인 변수들을 묶어서
			한개의 새로운 자료형으로 관리할 수 있다.
		구조체 : 배열은 같은 자료형만 묶을 수 있으므로 여러 형태의 데이터를 한개의 배열로는
			관리할 수 없다.배열의 단점을 보완하기 위해 사용되는 자료형
		클래스 : 구조체는 데이터만 관리할 수 있다. 여기에 어떤 기능들(메소드)을 추가한 것이 클래스이다.
		
		
		
		
		절차지향과 procedural programming 객체지향  object oriented
		...
		클래스는 다시 일반클래스와 추상클래스로 나눌 수있다.
		
		크게보면 2개(클래스와 인터페이스), 작게보면 3개(일반클래스, 추상클래스, 인터페이스)로 나뉜다.
