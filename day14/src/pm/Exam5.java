...
/*
 		...
 		배열을 먼저 생성하고 이것을 List<E>로 래팽 wrapping 즉 포장만 해 놓은다.
 		다라서 내부 구조는 배열과 동일하므로 컬렉션 객체인데도 크기 변경이 불가능하다.
 		구현 클래스로 객체를 생성했을 때와 달리 데이터의 추가 삭제가 불가능해진다.
 		단, 크기가 변경하지 않는 데이터의 변경은 가능하다. 따라서 고정된 개수의
 		데이터를 저장하거나 활용할 때 사용된다.
