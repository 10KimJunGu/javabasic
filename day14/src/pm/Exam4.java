...
Queue<E> 컬렉션 인터페이스
		...
		우표 수집 컬렉션에는 우표만 넣듯 한 컬렉션에는 동일한 타입의 데이터만 모아둘 수 있다.
		배열도 동일한 타입의 데이터를 묶어 관리를 하지만 배열은 컬렉션이 아니다.
		배열과 컬렉션의 차이는 바로 데이터의 저장을 동적으로 관리할 수 있느냐는 것이다.
		배열은 생성 시점에 저장공간의 크기를 확정해야하고 나중에 변경할 수 없는 반면
		컬렉션은 데이터의 갯수에 따라 얼마든지 동적으로 변화할 수 있다.
		...
		또는 구조에 따라 클래스 또는 인터페이스를 설계하고 이렇게 설계되 ㄴ클래스와 인터페이스를 묶어놓은 개념이다.
		
		컬렉션 프레임워크는 컬렉션과 프레임워크가 조합된 개념으로
		리스트 스택 큐 트리 등 자료구조에 정렬 탐색 등의 알고리즘을 구조화해 놓은 프레임워크이다.
...
저장 용량을 동적으로 관리                                       클래스의 정의에 설계 원칙 또는 구조(철학)
		...
		따라서 null값을 입력하는 것 말고는 할 수 있는게 없다. null로 입력해도 여전히 그 크기는
		5가 된다.
		
		리스트으 ㅣ특징
		List<String> aList = new ArrayLest<>();
		aList.add("a");
		aList.add("b");
		aList.add("c");
		aList.add("d");
		...
		//리스트 생성
		List<E>는 인터페이스이기 때문에 객체를 스스로 생성할 수 없다. 따라서  객체를
		생성하기 위해서는 List<E>를 구현받아서 자식 클래스를 생성하고 그 생성한 자식 클래스를 이용해서
		객체를 생성해야 한다. 하지만 컬렉션 프레임워크를 이용할 때는 직접
